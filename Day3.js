...
const [wire1, wire2] = data
    .split('\n')
    .map(wire => wire
        .split(',')
        .map(d => {
            const parsedData = /(\w)(\d+)/g.exec(d);
            return {
                direction: parsedData[1],
                amount: parseInt(parsedData[2]),
            };
        })
    );
...
const movePoint = direction => {
    switch(direction) {
        case 'U':
            return pos => ({x: pos.x, y: pos.y + 1, s: pos.s + 1});
        case 'L':
            return pos => ({x: pos.x - 1, y: pos.y, s: pos.s + 1});
        case 'D':
            return pos => ({x: pos.x, y: pos.y - 1, s: pos.s + 1});
        case 'R':
            return pos => ({x: pos.x + 1, y: pos.y, s: pos.s + 1});
...
const getPoints = wire => {
    ...
    for(let i = 0; i < wire.length; i++) {
        const line = wire[i];
        const move = movePoint(line.direction);
        for (let j = 0; j < line.amount; j++) {
            pos = move(pos);
            points.push(pos);
        }
    }
...
const getIntersections = (wire1, wire2) => {
    ...
    points1.forEach((p, i) => {
        const inter = points2.find(p2 => p2.x === p.x && p.y === p2.y);

        if(inter) {
            intersections.push({x: p.x, y: p.y, s: p.s + inter.s});
        }
    });
...
const solve = (w1, w2) => {
    return getIntersections(w1, w2).map(i => i.s).sort((a, b) => a - b)[0];
};

console.log(solve(wire1, wire2));